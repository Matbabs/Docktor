<script>
    ...
    import { localDockerImages, localDockerImagesIsLoading } from "../../stores/docker-images";
        ...
        <div class="menu-items">
                ...
                {#if $localDockerImages.length}
                    <div in:fade class="table body">
                        {#each $localDockerImages as image}
                            <span on:click={() => handleScan('local', `${image.Name}:${image.Tag}`)} class="selectable yellow" style="font-weight: bold;">{image.Id}</span>
            ...
            <div class="remote card light">
                ...
                <div class="remote-search">
                    <form on:submit|preventDefault={() => handleScan('remote', remoteSearch)}>
                        <input type="text" bind:value={remoteSearch} placeholder="docker image">
