            ...
            <div class="results card light">
                <div class="title">
                    ...
                    {:else if totalMisconfig === 0}
                        <h2>Scan Results - <span class={`${totalVulnStats === 0 ? 'green' : 'red'}`}>{totalVulnStats} Vulnerabilities</span></h2>
                    {:else}
                        <h2>Scan Results - <span class={`${totalMisconfig === 0 ? 'green' : 'red'}`}>{totalMisconfig} Misconfigurations</span></h2>
                    {/if}
                </div>
                <hr>
                {#if totalMisconfig === 0}
                        ...
                        {#each $scan.Results as result}
                            {#if result.Vulnerabilities}
                                {#each result.Vulnerabilities as vuln}
                                    <span title={vuln.VulnerabilityID} on:click={() => handleChangeVuln(vuln)} 
                                        class="selectable red">
                                        {vuln.VulnerabilityID}
                                    </span>
                                    <span class="ellipsis" title={vuln.PkgName}>{vuln.PkgName}</span>
                                    <span class="ellipsis" title={vuln.InstalledVersion}>{vuln.InstalledVersion}</span>
                                    <span style="font-weight: bold;" 
                                        class={`${stats.find(s => s.name === vuln.Severity).color}`}
                                        title={vuln.Severity}>
                                        {vuln.Severity}
                                    </span>
                                {/each}
                            ...
                            {#if result.Misconfigurations}
                                {#each result.Misconfigurations as misconfig}
                                    <span title={misconfig.ID} on:click={() => handleChangeVuln(misconfig)} 
                                        class="selectable red">
                                        {misconfig.ID}
                                    </span>
                                    <span class="ellipsis" title={misconfig.Title}>{misconfig.Title}</span>
                                    <span class="ellipsis" title={misconfig.Status}>{misconfig.Status}</span>
                                    <span style="font-weight: bold;" 
                                        class={`${stats.find(s => s.name === misconfig.Severity).color}`}
                ...
                {:else if totalVulnStats === 0 && totalMisconfig === 0}
                    <div class="no-data">
                        ...
                        <h3 class="green">No {totalMisconfig === 0 ? 'vulnerability' : 'misconfiguration'} have been founded</h3>
                    </div>
                {:else}
                    <div class="no-data">
                        <div class="icon">
                        ...
                        <h3>No {totalMisconfig === 0 ? 'vulnerability' : 'misconfiguration'}  have been founded</h3>
                    </div>
                {/if}
            </div>
        </div>
        <div class="report">
            <div class="stats">
                {#each stats as stat }
                    <div class={`stat card ${stat.color}-bg-light`}>
                        <div class={`status ${stat.color}-bg`} style={`height: ${stat.value*100/(totalMisconfig === 0 ? totalVulnStats : totalMisconfig)}%;`}/>
                        <div class="info">
                            <span>{stat.name}</span>
            ...
            <div class="vuln card light">
                <div class="title">
                    ...
                    {:else}
                        <h2>{totalMisconfig === 0 ? 'Vulnerability' : 'Misconfiguration'} Review</h2>
                    {/if}
                </div>
                <hr>
                {#if vuln}
                    <div in:fade class="details">
                        <div class={`alert ${stats.find(s => s.name === vuln.Severity).color}-bg-light`}>
                            <h3>{totalMisconfig === 0 ? vuln.VulnerabilityID : vuln.ID}</h3>
                            <span>{totalMisconfig === 0 ? vuln.PublishedDate : "Misconfiguration"}</span>
                        </div>
                        <p>{vuln.Title}</p>
                ...
                {:else}
                    <div class="no-data">
                        ...
                        {:else}
                            <h3>No {totalMisconfig === 0 ? 'vulnerability' : 'misconfiguration'} to review</h3>
                        {/if}
                    </div>
                {/if}
            </div>
