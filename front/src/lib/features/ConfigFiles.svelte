<script>
  ...
  import FaToolbox from "svelte-icons/fa/FaToolbox.svelte";

  let breadcrumb = ["Config Files"];
  ...
  $: if ($history) {
    filterHistory = $history.filter((h) => h.type === "config-files");
  }
    ...
    if ($fileSystem.stdout) {
      $fileSystem.stdout.forEach((line) => {
        if (line !== "") {
          ...
          repartition.set(
            type,
            repartition.has(type) ? repartition.get(type) + 1 : 1
          );
        }
      });
    }
  ...
  function handleScan(path, save = true) {
    ...
    scan.getScanConfig(path);
    navigate("/scan-report");
    if (save) {
      history.saveScan(path, "config-files");
    }
  }
</script>
    ...
    <div class="menu-items">
      <div class="local card light">
        <div class="title">
          ...
          <h2>Local Config Files - Misconfigurations Analysis</h2>
        </div>
        <hr />
        <div class="scan-tool">
          <div>
            ...
            {#if $fileSystem}
              ...
              {#each $fileSystem.stdout as line}
                {#if !line.includes(".") && line != "" && line != ".." && line != "."}
                  <span in:fade class="clickable" on:click={() => addPath(line)}
                    >/{line}</span
                  >
                {:else if line != ".." && line != "."}
